...
class Product:
    def __init__(self):
        ...
        self.name = ''
        self.path = '\\\\192.168.1.155\\!!! Druk !!!\\2020\\12.2020\\'
    ...
    def prepare_data(self):
        ...
        if self.name[:3] == '720' or self.name[:5] == 'R_720':
            ...
            print('Enter printer type UV 360 720')
            c = input()
            if c == '720':
                self.path += '720\\'
                self.df = pd.read_csv('eco.csv')
    ...
    def get_material(self):
        ...
        if 'lyt' in name or 'lut' in name or ('lit' in name and 'beklit' not in name) or 'banLYT' in name or 'лит' in name:
            return self.df[self.df['Матеріал'] == 'Банерна тканина «Frontlit» лита']
                                  ...
                                  and 'solex' not in name) or 'лам' in name:
            return self.df[self.df['Матеріал'] == 'Банерна тканина «Frontlit» ламінована']
        elif 'sitka' in name or 'citka' in name or 'сітка' in name:
            return self.df[self.df['Матеріал'] == 'Сітка банерна']
        elif ('gl' in name or 'mat' in name or 'proz' in name) and 'holst' not in name or 'гл' in name or 'мат' in name:
        ...
        elif 'siti' in name or 'сітік' in name or 'citi' in self.name or 'сіті' in self.name or 'sity' in self.name:
            return self.df[self.df['Матеріал'] == 'Папір для друку постерний']
        elif 'vision' in name or 'visiomn' in name:
            return self.df[self.df['Матеріал'] == 'Самоклеюча плівка «One Way Vision»']
        elif 'blu' in name:
            return self.df[self.df['Матеріал'] == 'Папір Blueback']
        else:
            return self.df[self.df['Матеріал'] == 'Unknown']
    ...
    def calculate(self):
        ...
        if self.material['Матеріал'].values[0] in ('Самоклеюча плівка Ritrama', 'Самоклеюча плівка Китайська',
            ...
            if 'lam' in (self.path + self.name).lower():
